...
import { MINIMARKET_PROGRAM_ADDRESS } from '../programs';
...
export type DecrementInstruction<
  TProgram extends string = typeof MINIMARKET_PROGRAM_ADDRESS,
  TAccountMinimarket extends string | AccountMeta<string> = string,
  TRemainingAccounts extends readonly AccountMeta<string>[] = [],
> = Instruction<TProgram> &
  InstructionWithData<ReadonlyUint8Array> &
  InstructionWithAccounts<
    [
      TAccountMinimarket extends string
        ? WritableAccount<TAccountMinimarket>
        : TAccountMinimarket,
      ...TRemainingAccounts,
    ]
  >;
...
export type DecrementInput<TAccountMinimarket extends string = string> = {
  minimarket: Address<TAccountMinimarket>;
};

export function getDecrementInstruction<
  TAccountMinimarket extends string,
  TProgramAddress extends Address = typeof MINIMARKET_PROGRAM_ADDRESS,
>(
  input: DecrementInput<TAccountMinimarket>,
  config?: { programAddress?: TProgramAddress }
): DecrementInstruction<TProgramAddress, TAccountMinimarket> {
  // Program address.
  const programAddress = config?.programAddress ?? MINIMARKET_PROGRAM_ADDRESS;

  // Original accounts.
  const originalAccounts = {
    minimarket: { value: input.minimarket ?? null, isWritable: true },
  };
  ...
  const instruction = {
    accounts: [getAccountMeta(accounts.minimarket)],
    programAddress,
    data: getDecrementInstructionDataEncoder().encode({}),
  } as DecrementInstruction<TProgramAddress, TAccountMinimarket>;
